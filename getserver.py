...
def Launch():
    ...
    while not hasFound:
        ...
        if line:
            print(line)
            match = re.search(r"UDMUX Address = (\d+\.\d+\.\d+\.\d+), Port = (\d+)", line)
            if match:
            ...
            match = re.search(r"RCC Server Address = (\d+\.\d+\.\d+\.\d+), Port = (\d+)", line)
            if match:
            ...
            if not hasFound:
                NotifyPlayer("Roblox Configurator", "Failed to get server information\nReason: No more log output")
                if found_rcc and found_udmux:
                    hasFound = True
                    break
                if found_udmux:
                    NotifyPlayer("Roblox Configurator", f"Failed to get RCC\n Server IP : {udmux_address}\nServer Port : {udmux_port}")
                if found_rcc:
                    NotifyPlayer("Roblox Configurator", f"Failed to get Server\nRCC IP : {rcc_server_address}\nRCC Port: {rcc_server_port}")
            time.sleep(5)
            break
